# Welcome to Statistics 224 Documentation

!!! info "Complete Reference Guide"
    This comprehensive documentation provides everything you need for Statistics 224, from fundamental concepts to advanced statistical analyses.

## 🚀 Quick Navigation

### First Time Here?
→ Start with [Getting Started](quick-start/getting-started.md)  
→ Set up [R and Posit Cloud](quick-start/r-setup.md)  
→ Try the [Interactive Modules](modules/overview.md)

### Need to Run a Statistical Test?
→ Use the [Decision Tree](which-test/decision-tree.md) to find the right test  
→ Check [Common Scenarios](which-test/scenarios.md) for examples  
→ Jump to the specific test workflow

### Checking Assumptions?
→ See [Effect Sizes Reference](effect-sizes/reference.md)  
→ Learn about diagnostics in each test section

### Need Practice?
→ Explore [Interactive Learning Modules](modules/overview.md)  
→ Try the [Normality Series](modules/normality.md)  
→ Work through [Regression Modules](modules/regression.md)

---

## 📚 Main Sections

### [Parametric Tests](parametric/t-tests/overview.md)
Complete workflows for:

- **T-Tests**: One-sample, Independent, Paired
- **ANOVA**: One-way, Two-way, Repeated Measures
- **Regression**: Simple, Multiple, Diagnostics

### [Non-Parametric Tests](non-parametric/overview.md)
Alternatives when assumptions fail:

- Mann-Whitney U Test
- Kruskal-Wallis Test
- Wilcoxon Signed-Rank Test

### [Chi-Square Analysis](chi-square/overview.md)
For categorical data:

- Test of Independence
- Goodness of Fit Test

---

## 🎓 Interactive Learning

### [11+ Browser-Based Modules](modules/overview.md)

**Normality Series** (5 modules)  
Master assumption checking through progressive learning

**Regression Series** (4 modules)  
Build intuition with dynamic visualizations

**Chi-Square & Probability**  
Hands-on categorical data analysis and sampling distributions

[Visit Module Library →](https://jkoxford-a11y.github.io/solid-carnival/)

---

## 📊 Key Features

!!! success "What Makes This Documentation Special"
    ✓ **Definitive Workflows** - One authoritative guide per test  
    ✓ **Decision Trees** - Visual guides for test selection  
    ✓ **Effect Size Reference** - Interpretation at your fingertips  
    ✓ **Code Examples** - Copy-paste R workflows  
    ✓ **Troubleshooting** - Real error messages with solutions  
    ✓ **Interactive Modules** - Hands-on learning tools  
    ✓ **Reporting Templates** - APA-style examples  

---

## 🔍 How to Use This Documentation

### For Quick Reference
Use the **search bar** above to find any topic instantly

### For Learning
Follow the sections in order, working through examples

### For Analysis
1. [Which Test?](which-test/decision-tree.md) - Choose your test
2. Navigate to that test's workflow
3. Copy and adapt the R code
4. Check [Effect Sizes](effect-sizes/reference.md) for interpretation

### For Teaching
- Assign [Interactive Modules](modules/overview.md) before class
- Reference specific sections in assignments
- Use [Common Scenarios](which-test/scenarios.md) for examples

---

## 📖 About This Documentation

**Course**: Statistics 224 - Statistical Methods  
**Institution**: Columbia College of Missouri  
**Author**: Jon Oxford, Associate Professor of Psychology

This documentation integrates:

- Comprehensive statistical theory
- Practical R workflows
- Interactive learning modules
- Professional best practices

**Last Updated**: November 2024

---

## 🆘 Need Help?

- **Can't find a topic?** Use the search bar above
- **Unsure which test?** Try the [Decision Tree](which-test/decision-tree.md)
- **Assumptions failing?** See test-specific troubleshooting sections
- **Want practice?** Explore [Interactive Modules](modules/overview.md)

---

!!! tip "Pro Tip"
    Bookmark this page! You'll reference it frequently throughout the course.

**Ready to begin?** → [Get Started](quick-start/getting-started.md)
